...
function statusVariant(status: string) {
  if (["completed", "approved", "in_transit"].includes(status)) {
    return "default";
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Move detergent stock between warehouses using the backend transfer
            workflow: create, approve, complete, and cancel.
          </p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">Draft on page</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">Completed on page</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <Card>
        <CardHeader className="space-y-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Backend endpoint: /stock-transfers.
            </p>
          </div>
          ...
          <Table>
            ...
            <TableBody>
              {transfersQuery.isLoading ? (
                <TableRow>
                  <TableCell colSpan={6}>Loading transfers...</TableCell>
                </TableRow>
              ) : rows.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={6}>No transfers found.</TableCell>
                </TableRow>
              ) : (
                rows.map((row) => (
                  <TableRow key={row.id}>
                    <TableCell>
                      <div className="font-medium">
                        {row.transfer_number ?? `TRF-${row.id}`}
                      </div>
        ...
        <DialogContent className="max-h-[90vh] w-[calc(100vw-2rem)] max-w-2xl overflow-x-hidden overflow-y-auto p-4 sm:p-6">
          ...
          <div className="grid min-w-0 gap-4 sm:grid-cols-2">
            ...
            <div className="min-w-0 space-y-2 sm:col-span-2">
              <Label>Notes</Label>
              <Textarea
                className="min-h-24 w-full min-w-0 resize-y break-words"
                value={form.notes}
                onChange={(event) =>
                  setForm((current) => ({
                    ...current,
